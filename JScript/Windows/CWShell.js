...
// Use aMsg.stdout to get value from std::out
// Use aMsg.stderr to get value from std::err
CWUtils.Exec = CWUtils.Exec || function (aCmd, aWaitEnd, aMsg, aPrintMsg)
{
    ...
    if (true == aWaitEnd)
    {
        ...
        while (0 == execObj.Status)   //0 for running, 1 for completed
        {
            ...
            while (false == execObj.StdOut.AtEndOfStream)
            {
                ...
                if (typeof aMsg !== "undefined")
                {
                    if (aMsg.stdout.length > 0)
                    {
                        aMsg.stdout += "\n";
                    }
                    aMsg.stdout += strOutput;
                }
            }
            //It's needed to clean StdErr anyway or the execObj.Status will not changed to completed forever
            while (false == execObj.StdErr.AtEndOfStream)
            {
                strOutput = execObj.StdErr.ReadLine();
                if (aPrintMsg)
                {
                    WScript.Echo(strOutput);
                }
                if (typeof aMsg !== "undefined")
                {
                    if (aMsg.stderr.length > 0)
                    {
                        aMsg.stderr += "\n";
                    }
                    aMsg.stderr += strOutput;
                }
            }
        }
        while (false == execObj.StdOut.AtEndOfStream)
