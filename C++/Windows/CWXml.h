...
#define CW_XML_SHOW_ERR( aHR, aCode )                                            \
    do                                                                           \
    {                                                                            \
        aHR = aCode;                                                             \
        if ( FAILED( aHR ) )                                                     \
        {                                                                        \
            wprintf_s( L"[%hs][Line %lu][%ws]\n", __FILE__, __LINE__, L#aCode ); \
            goto exit;                                                           \
        }                                                                        \
    } while ( 0 )
...
class CWXml
{
    ...
    public:
    HRESULT ParserXmlFile( IN CONST WCHAR * aXmlPath,
                           IN OUT CWXmlElementNode * aRootNode,
                           IN DWORD aCodePage = CP_UTF8 );
