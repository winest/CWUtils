...
BOOL CDllInjectClient::Connect( CONST WCHAR * aKey,
                                PFN_DLL_INJECT_CLIENT_INIT_CBK aInitCbk,
                                PFN_DLL_INJECT_CLIENT_DATA_RECV_CBK aDataCbk )
{
    ...
    if ( this->IsConnected() )
    {
        ...
        {
            DbgOut( ERRO, DBG_DLL_INJECT_MGR, "Shared memory already used by others. m_wstrKey=%ws, aKey=%ws",
                    m_wstrKey.c_str(), aKey );
    ...
    if ( !hSmInit )
    {
        DbgOut( ERRO, DBG_DLL_INJECT_MGR, "OpenFileMappingW() with name %ws failed. GetLastError()=%!WINERROR!", aKey,
                GetLastError() );
        goto exit;
    }
    ...
    if ( !pSmInit )
    {
        DbgOut( ERRO, DBG_DLL_INJECT_MGR, "MapViewOfFile() with name %ws failed. GetLastError()=%!WINERROR!", aKey,
                GetLastError() );
        goto exit;
    }
    ...
    if ( NULL == pSmInit->InitReq.Local.pLocalCtx || NULL == pSmInit->InitReq.Local.hRemoteProc ||
         NULL == pSmInit->InitReq.Local.pfnFreeLibrary )
    {
    ...
    m_SmData[PER_SERVER_SM_INDEX_REMOTE_TO_LOCAL] = (PDLL_INJECT_SERVER_SM_DATA_HEADER)MapViewOfFile(
        (HANDLE)pSmInit->InitReq.Remote.hPerServerSm[PER_SERVER_SM_INDEX_REMOTE_TO_LOCAL], FILE_MAP_ALL_ACCESS, 0, 0,
        0 );
    if ( NULL == m_SmData[PER_SERVER_SM_INDEX_REMOTE_TO_LOCAL] )
    {
        DbgOut( ERRO, DBG_DLL_INJECT_MGR,
                "MapViewOfFile() with PER_SERVER_SM_INDEX_REMOTE_TO_LOCAL failed. GetLastError()=%!WINERROR!",
                GetLastError() );
        pSmInit->InitRsp.dwHookStatus = GetLastError();
        goto exit;
    }
    m_SmData[PER_SERVER_SM_INDEX_LOCAL_TO_REMOTE] = (PDLL_INJECT_SERVER_SM_DATA_HEADER)MapViewOfFile(
        (HANDLE)pSmInit->InitReq.Remote.hPerServerSm[PER_SERVER_SM_INDEX_LOCAL_TO_REMOTE], FILE_MAP_ALL_ACCESS, 0, 0,
        0 );
    if ( NULL == m_SmData[PER_SERVER_SM_INDEX_LOCAL_TO_REMOTE] )
    {
        DbgOut( ERRO, DBG_DLL_INJECT_MGR,
                "MapViewOfFile() with PER_SERVER_SM_INDEX_LOCAL_TO_REMOTE failed. GetLastError()=%!WINERROR!",
                GetLastError() );
        pSmInit->InitRsp.dwHookStatus = GetLastError();
        goto exit;
    }
    ...
    DbgOut( VERB, DBG_DLL_INJECT_MGR,
            "Remote.hPerClientEvt[PER_CLIENT_EVT_INDEX_STOP]=0x%p, "
            "Remote.hPerClientEvt[PER_CLIENT_EVT_INDEX_REMOTE_REQ_OK]=0x%p, "
            "Remote.hPerClientEvt[PER_CLIENT_EVT_INDEX_REMOTE_RSP]=0x%p",
            (HANDLE)pSmInit->InitReq.Remote.hPerClientEvt[PER_CLIENT_EVT_INDEX_STOP],
            (HANDLE)pSmInit->InitReq.Remote.hPerClientEvt[PER_CLIENT_EVT_INDEX_REMOTE_REQ_OK],
            (HANDLE)pSmInit->InitReq.Remote.hPerClientEvt[PER_CLIENT_EVT_INDEX_REMOTE_RSP] );
    ...
    if ( aDataCbk )
    {
        ...
        m_hDataRecvThread =
            (HANDLE)_beginthreadex( NULL, 0, CDllInjectClient::DataRecvThread, this, 0, &uDataThreadId );
        if ( NULL == m_hDataRecvThread )
        {
            DbgOut( ERRO, DBG_DLL_INJECT_MGR,
                    "_beginthreadex() with for DataRecvThread failed. GetLastError()=%!WINERROR!", GetLastError() );
            pSmInit->InitRsp.dwHookStatus = GetLastError();
            goto exit;
        }
    ...
    pSmInit->InitRsp.dwHookStatus =
        ( aInitCbk ) ? aInitCbk( pSmInit->InitReq.wzServerDirPath, pSmInit->InitReq.wzClientCfgPath ) : ERROR_SUCCESS;
...
exit:
    ...
    {
        DbgOut( ERRO, DBG_DLL_INJECT_MGR, "UnmapViewOfFile() pSmInit failed. GetLastError()=%!WINERROR!",
                GetLastError() );
    }
...
BOOL CDllInjectClient::SendData( CHAR * aReqBuf, DWORD aReqBufSize, CHAR * aRspBuf, DWORD * aRspBufSize )
{
    _ASSERT( m_SmInit.InitReq.Remote.hDllInjectMgrAliveThread &&
             ...
             m_SmInit.InitReq.Local.pLocalCtx && m_SmData[PER_SERVER_SM_INDEX_REMOTE_TO_LOCAL] &&
    ...
    HANDLE hWaitInstance[] = {
        (HANDLE)m_SmInit.InitReq.Remote.hPerServerEvt[PER_SERVER_EVT_INDEX_STOP],
    ...
    switch ( dwWaitReqOk )
    {
        ...
        {
            DbgOut( ERRO, DBG_DLL_INJECT_MGR,
                    "WaitForMultipleObjects() return unexpected value 0x%08X. GetLastError()=%!WINERROR!", dwWaitReqOk,
                    GetLastError() );
            break;
        }
    }
    ...
    switch ( dwWaitRsp )
    {
        ...
        case WAIT_ABANDONED_0 + 3:
        {
            ...
            DbgOut( INFO, DBG_DLL_INJECT_MGR, "Scan result is %!HEXDUMP!",
...
UINT CALLBACK CDllInjectClient::DataRecvThread( VOID * pThis )
{
    ...
    {
        DbgOut( ERRO, DBG_DLL_INJECT_MGR,
                "DataRecv exit abnormally. Try to set stop event. uRet=%!WINERROR!. GetLastError()=%!WINERROR!", uRet,
                GetLastError() );
...
DWORD CDllInjectClient::DoDataRecv()
{
    _ASSERT( m_SmInit.InitReq.Remote.hDllInjectMgrAliveThread &&
             ...
             m_SmInit.InitReq.Local.pLocalCtx && m_SmData[PER_SERVER_SM_INDEX_REMOTE_TO_LOCAL] &&
             m_SmData[PER_SERVER_SM_INDEX_LOCAL_TO_REMOTE] && NULL != m_pfnDataRecv );
        ...
        switch ( dwWaitData )
        {
            ...
            case WAIT_OBJECT_0 + 3:    //PER_CLIENT_EVT_INDEX_LOCAL_REQ
            {
                ...
                if ( NULL != pReq->pReq && 0 < dwReqSizeCopy )
                {
                    ...
                    if ( NULL == pReqCopy )
                    {
                        DbgOut( ERRO, DBG_DLL_INJECT_MGR, "Failed to allocate memory for pReqCopy. dwReqSizeCopy=%u",
                                dwReqSizeCopy );
                ...
                switch ( dwWaitSmR2L )
                {
                    ...
                    case WAIT_ABANDONED_0 + 3:
                    {
                        ...
                        DLL_INJECT_SERVER_SM_DATA_GENERAL_RSP * pRsp =
                            (DLL_INJECT_SERVER_SM_DATA_GENERAL_RSP *)&pSmR2L->pData;
                        ...
                        DWORD dwWaitRspOk =
                            WaitForMultipleObjects( _countof( hWaitRspOk ), hWaitRspOk, FALSE, INFINITE );
                        switch ( dwWaitRspOk )
                        {
                            case WAIT_OBJECT_0:    //PER_SERVER_EVT_INDEX_STOP
                            case WAIT_ABANDONED_0:
                            ...
                            {
                                DbgOut( ERRO, DBG_DLL_INJECT_MGR,
                                        "WaitForMultipleObjects() return unexpected value 0x%08X. "
                                        "GetLastError()=%!WINERROR!",
                                        dwWaitRspOk, GetLastError() );
                                dwRet = ERROR_INVALID_HANDLE_STATE;
                                bStop = TRUE;
                                break;
                            }
