...
BOOL CDynamicBuffer::ExtendBufferIfNeed( SIZE_T aExtendSize, UCHAR ** aOldBuf )
{
    *aOldBuf = NULL;
    ...
    {
        SIZE_T ulNewAllocSize = max( ( SIZE_T )( (DOUBLE)m_ulAllocSize * m_fGrowRate ), m_ulAllocSize + aExtendSize ) +
...
BOOL CDynamicBuffer::ShrinkBufferIfNeed( SIZE_T aShrinkSize, UCHAR ** aOldBuf )
{
    *aOldBuf = NULL;
    ...
    {
        SIZE_T ulNewAllocSize =
            max( SIZE_T( ( DOUBLE )( m_ulCurSize - aShrinkSize ) / m_fShrinkRate ), ( m_ulCurSize - aShrinkSize ) ) +
            1;    //+1 for terminating NULL
...
BOOL CMemFileBuffer::WriteToFile( CONST UCHAR * aBuf, SIZE_T aBufSize )
{
    ...
    {
        m_hFile = CreateFileW( m_wstrFilePath.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL,
