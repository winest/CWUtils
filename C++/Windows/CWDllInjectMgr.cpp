...
BOOL CDllInjectMgr::Init( CONST WCHAR * aCfgPath )
{
    ...
    do 
    {
        ...
        {
            DbgOut( ERRO , DBG_DLL_INJECT_MGR , "RelativeToFullPath() failed. GetLastError()=%!WINERROR!" , GetLastError() );
            break;
        }

        if ( FALSE == this->GetFunctionAddresses() )
        {
            DbgOut( ERRO , DBG_DLL_INJECT_MGR , "GetFunctionAddresses() failed. GetLastError()=%!WINERROR!" , GetLastError() );
            break;
        }
...
BOOL CDllInjectMgr::GetConfig( CONST WCHAR * aRuleName , DLL_INJECT_SERVER_CFG_TYPE aCfgType , IN OUT VOID * pData , IN OUT UINT * uDataSize )
{
...
BOOL CDllInjectMgr::ChangeConfig( CONST WCHAR * aRuleName , DLL_INJECT_SERVER_CFG_TYPE aCfgType , IN VOID * pData , IN UINT uDataSize )
{
    DbgOut( VERB , DBG_DLL_INJECT_MGR , "Enter. aRuleName=%ws , aCfgType=%d" , aRuleName , aCfgType );
...
BOOL CDllInjectMgr::GetFunctionAddresses()
{
    ...
    do 
    {
            ...
            m_uLoadLibraryW64 = (DWORD) GetProcAddress( GetModuleHandleW(KERNEL32_MODULE_NAME) , LOADLIBRARY_FUNCTION_NAMEA );
            m_uFreeLibrary64 = (DWORD) GetProcAddress( GetModuleHandleW(KERNEL32_MODULE_NAME) , FREELIBRARY_FUNCTION_NAMEA );
            DbgOut( INFO , DBG_DLL_INJECT_MGR , "m_uLoadLibraryW64=0x%0I64X, m_uFreeLibrary32=0x%0I64X" , m_uLoadLibraryW64 , m_uFreeLibrary64 );
            ...
            _snwprintf_s( wzDllInjectMgrHelperLoadLibrary , _TRUNCATE , L"\"%ws\" %ws" , wstrDllInjectMgrHelper.c_str() , LOADLIBRARY_FUNCTION_NAMEW );
            _snwprintf_s( wzDllInjectMgrHelperFreeLibrary , _TRUNCATE , L"\"%ws\" %ws" , wstrDllInjectMgrHelper.c_str() , FREELIBRARY_FUNCTION_NAMEW );
            if ( FALSE == CWUtils::ExecuteCommandLine( wzDllInjectMgrHelperLoadLibrary , FALSE , (DWORD *)&m_uLoadLibraryW32 ) )
            {
                DbgOut( ERRO , DBG_DLL_INJECT_MGR , "Failed to get response for command \"%ws\"" , wzDllInjectMgrHelperLoadLibrary );
                break;
            }
            if ( FALSE == CWUtils::ExecuteCommandLine( wzDllInjectMgrHelperFreeLibrary , FALSE , (DWORD *)&m_uFreeLibrary32 ) )
            {
                DbgOut( ERRO , DBG_DLL_INJECT_MGR , "Failed to get response for command \"%ws\"" , wzDllInjectMgrHelperFreeLibrary );
                break;
            }
        #else
            DbgOut( INFO , DBG_DLL_INJECT_MGR , "Get x86 address by GetProcAddress()" );
            m_uLoadLibraryW32 = (DWORD) GetProcAddress( GetModuleHandleW(KERNEL32_MODULE_NAME) , LOADLIBRARY_FUNCTION_NAMEA );
            m_uFreeLibrary32 = (DWORD) GetProcAddress( GetModuleHandleW(KERNEL32_MODULE_NAME) , FREELIBRARY_FUNCTION_NAMEA );
        #endif                   

        DbgOut( INFO , DBG_DLL_INJECT_MGR , "m_uLoadLibraryW32=0x%08X, m_uFreeLibrary32=0x%08X" , m_uLoadLibraryW32 , m_uFreeLibrary32 );
