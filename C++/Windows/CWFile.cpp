        ...
        if ( byVerInfoBuf != NULL )
        {
            if ( ::GetFileVersionInfoW( aFullPath, dwVerHandle, dwVerInfoSize, byVerInfoBuf ) )
            {
                ...
                if ( ::VerQueryValueW( byVerInfoBuf, L"\\", (LPVOID *)&tempVerBuf, &uTempBufLen ) )
                {
                    if ( aMajor )
                    {
                        *aMajor = HIWORD( tempVerBuf->dwFileVersionMS );
                    }
                    if ( aMinor )
                    {
                        *aMinor = LOWORD( tempVerBuf->dwFileVersionMS );
                    }
                    if ( aRevision )
                    {
                        *aRevision = HIWORD( tempVerBuf->dwFileVersionLS );
                    }
                    if ( aBuildNumber )
                    {
                        *aBuildNumber = LOWORD( tempVerBuf->dwFileVersionLS );
                    }
                    bSuccess = TRUE;
                }
            }
...
VERSION_COMPARE CompareFileVersion( CONST WCHAR * aNewFilePath, CONST WCHAR * aExistFilePath )
{
         ...
         GetFileVersion( aExistFilePath, &wExistMajor, &wExistMinor, &wExistRev, &wExistBuild ) )
    {
        ...
        wNew[2] = wNewRev;
        wNew[3] = wNewBuild;
        wExist[0] = wExistMajor;
        wExist[1] = wExistMinor;
        wExist[2] = wExistRev;
...
BOOL GetFilePathFromHandle( IN HANDLE aFile, OUT wstring & aFilePath )
{
    ...
    for ( ;; )
    {
        ...
        if ( IsWindowsVistaOrGreater() )
        {
            ...
            DWORD dwNewPathLen =
                GetFinalPathNameByHandleW( aFile, wzPath, _countof( wzPath ) - 1, FILE_NAME_NORMALIZED );
            if ( 0 == dwNewPathLen )
            {
                break;
            }
            else if ( _countof( wzPath ) <= dwNewPathLen )
            {
                ...
                {
                    if ( dwNewPathLen >=
                         GetFinalPathNameByHandleW( aFile, wzNewPath, dwNewPathLen - 1, FILE_NAME_NORMALIZED ) )
                    {
                        aFilePath = wzNewPath;
                        bRet = TRUE;
                    }
...
BOOL IsEmptyDir( CONST WCHAR * aDirPath )
{
    ...
    {
        if ( find.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY && ( wcsncmp( find.cFileName, L".", 1 ) == 0 ) ||
             ( wcsncmp( find.cFileName, L"..", 2 ) == 0 ) )
        {
            count++;
...
BOOL CreateDir( CONST wstring & aDir )
{
    ...
    for ( i = 0; i < tokens.size(); i++ )
    {
        ...
        HANDLE hDir = CreateFileW( path.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING,
...
BOOL CFile::Open( CONST WCHAR * aPath, UINT32 aOpenAttr, CONST std::string aLineSep )
{
    ...
    do
    {
        ...
        HANDLE hFile =
            CreateFileW( aPath, dwAccess, FILE_SHARE_READ, NULL, dwCreateDisposition, FILE_ATTRIBUTE_NORMAL, NULL );
        if ( INVALID_HANDLE_VALUE == hFile )
        {
            break;
        }
