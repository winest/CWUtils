...
class CFileMonitorRequest
{
    ...
    VOID PushToQueue(
        CFileMonitor * aMonitor,
        CWFileMonitorNotification & aNotification );       //Push notification data to queue for later processing
    ...
    wstring GetNotifyPath( CONST FILE_NOTIFY_INFORMATION *
                               aNotifyInfo );    //Convert the FileName in FILE_NOTIFY_INFORMATION to the full path
    ...
    vector<BYTE>
        m_vecShareMemBak;    //Used to backup share memory once completion routine is called so we can receive next notification as soon as possible
};
...
class CFileMonitor
{
    ...
    CFileMonitor() :
        ...
        m_hThreadDispatcher( NULL ),
        m_uTotalThread( 0 ),
        m_uTotalWork( 0 ),
        m_fnOnFileChanged( NULL )
    {
    }
    ~CFileMonitor() { Stop(); }
    ...
    BOOL Start( CONST WCHAR * aMonitorPath,
                BOOL aRecursive,
                DWORD aFilter = FILE_NOTIFY_CHANGE_FILE_NAME | FILE_NOTIFY_CHANGE_DIR_NAME |
