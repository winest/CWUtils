...
VOID CFileMonitorRequest::StartCheck( ULONG_PTR aRequest )
{
    ...
    {
        req->m_hMonitorPath = CreateFileW( req->m_wstrMonitorPath.c_str() , FILE_LIST_DIRECTORY , FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE , NULL , 
                                           OPEN_EXISTING , FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OVERLAPPED , NULL );
        if ( INVALID_HANDLE_VALUE == req->m_hMonitorPath )
            return;           
    }
...
VOID CALLBACK CFileMonitorRequest::OnChangeComplete( DWORD aErrorCode , DWORD aBytesTransfered , LPOVERLAPPED aOverlapped )
{
...
VOID CFileMonitorRequest::ProcessNotification()
{
    ...
    for (;;)
    {
        ...
        switch ( fni->Action )
        {
            ...
            {
                if ( 0 != fni->NextEntryOffset ) //We assume RENAMED_OLD always immediately followed by the RENAME_new (std::nothrow) event
                {
                    CWFileMonitorNotification notification;
                    notification.dwEvent = GetNotifyEvent( fni );
                    notification.wstrSrcPath = GetNotifyPath( fni );
...
BOOL CFileMonitor::NormalizeParam( CONST WCHAR * aMonitorPath , BOOL & aRecursive , DWORD & aFilter , DWORD & aShareMemSize )
{
    BOOL bRet = FALSE;
    ...
    else
    {
        ...
        HANDLE hPath = CreateFileW( aMonitorPath , FILE_LIST_DIRECTORY , FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE , NULL , 
                                    OPEN_EXISTING , FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OVERLAPPED , NULL );
...
BOOL CFileMonitor::CreateMonitorDispatcherThread()
{
    ...
    if ( NULL == m_hThreadMonitor )
    {
        ...
        m_hThreadMonitor = (HANDLE)_beginthreadex( NULL , 0 , CFileMonitor::MonitorThread , static_cast<VOID *>(this) , NULL , NULL );
    ...
    {
        InterlockedIncrement( &m_uTotalThread );
        m_hThreadDispatcher = (HANDLE)_beginthreadex( NULL , 0 , CFileMonitor::DispatcherThread , static_cast<VOID *>(this) , NULL , NULL );
