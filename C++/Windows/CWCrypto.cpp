...
BOOL _GetLastErrorStringW( IN OUT std::wstring & aString , WORD aLang = MAKELANGID(LANG_ENGLISH,SUBLANG_ENGLISH_US) )
{
    BOOL bRet = FALSE;
    WCHAR * wzMsgBuf;
    SIZE_T sizeLen = (SIZE_T)FormatMessageW( FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM , NULL , GetLastError() , 
                                             aLang , (WCHAR *)&wzMsgBuf , 0 , NULL );
    if ( 0 < sizeLen )
    {
...
VOID Base64Encode( CONST UCHAR * aInput , SIZE_T aInputSize , std::string & aOutput , CONST CHAR * aEncodeTable )
{
    ...
    CONST CHAR * pEncTable = ( aEncodeTable ) ? aEncodeTable : "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
...
VOID Base64Decode( CONST UCHAR * aInput , SIZE_T aInputSize , std::string & aOutput , CONST CHAR * aDecodeTable )
{
    ...
    CONST CHAR * pDecTable = ( aDecodeTable ) ? aDecodeTable : "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
...
BOOL CRsa::CreateAndUseKey( CONST WCHAR * aContainer , CONST WCHAR * aProvider )
{
    ...
    do 
    {
        ...
        if ( FALSE == CryptGenKey( m_hProvider , CALG_RSA_KEYX , /*( 2048 << 16 )*/ RSA1024BIT_KEY | CRYPT_EXPORTABLE , &m_hKey ) )
        {
            _GetLastErrorStringW( wstrErr );
...
BOOL CRsa::ImportAndUseKey( CONST UCHAR * aKeyBlob , DWORD aKeyBlobSize , CONST WCHAR * aContainer , CONST WCHAR * aProvider )
{
    BOOL bRet = FALSE;
    this->CloseKey();
    ...
    {
        wstring wstrErr;
        if ( FALSE == CryptAcquireContextW( &m_hProvider , aContainer , aProvider , PROV_RSA_FULL , CRYPT_VERIFYCONTEXT | CRYPT_SILENT ) )
        {
            if ( NTE_BAD_KEYSET != GetLastError() )
            {
                _GetLastErrorStringW( wstrErr );
            ...
            if ( FALSE == CryptAcquireContextW( &m_hProvider , aContainer , aProvider , PROV_RSA_FULL , CRYPT_NEWKEYSET | CRYPT_VERIFYCONTEXT | CRYPT_SILENT ) )
            {
                _GetLastErrorStringW( wstrErr );
                wprintf_s( L"CryptAcquireContextW() failed. GetLastError()=0x%08X (%ws)\n" , GetLastError() , wstrErr.c_str() );
                break;
        ...
        if ( FALSE == CryptImportKey( m_hProvider , (CONST BYTE *)aKeyBlob , aKeyBlobSize , NULL , CRYPT_NO_SALT , &m_hKey ) )
        {
            _GetLastErrorStringW( wstrErr );
