...
typedef enum _HttpContentEncodingType
{
    HTTP_CONTENT_ENCODING_IDENTITY =
        0,    //No compress, should only specified in Accept-Encoding header rather than Content-Encoding
...
class CHttpFieldMgr
{
    ...
    virtual HttpParserErr SetField( HttpFieldId aFieldId,
                                    std::string & aData,
                                    UINT aDataSize,
                                    BOOL aAppend,
                                    BOOL aComplete );
    virtual HttpParserErr SetUnknownField( std::string aFieldName,
                                           std::string & aData,
                                           UINT aDataSize,
                                           BOOL aAppend,
                                           BOOL aComplete );
...
typedef BOOL( CALLBACK * ParserUpdateCallback )( IN VOID * aContext,
                                                 IN BOOL aConnOut,
                                                 IN HttpParserErr aParseRet,
                                                 OUT INT * aCallbackRet );
...
class CHttpParser
{
    ...
    BOOL Init( VOID * pCbkCtx,
               ParserUpdateCallback pfnHeaderCbk,
               ParserUpdateCallback pfnBodyCbk,
               UINT aMaxHeaderCnt = INFINITE,
               UINT aMaxBodySize = INFINITE );
    VOID Reset();
    VOID UnInit();
    ...
    class CIncompleteHeaderState
    {
        ...
        BOOL
            bIsIncompleteFieldName;    //An unknown field name is incomplete,if the field name is known, it should be hold in Tree pos.
        std::string
            strUnknownFieldName;    //Current unknown Field name. for identifying case, it is used to store field value;
    };
    ...
    UINT DoFieldSearch( CWUtils::CPrefixTree<UINT> & aMatcher,
                        const UCHAR * aBuf,
                        UINT aBufSize,
