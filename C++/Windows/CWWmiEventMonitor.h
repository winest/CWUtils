...
{
    public :
        CWmiEventMonitor() : m_lRef(0) , m_pLoc(NULL) , m_pSvc(NULL) , m_pUnsecApp(NULL) ,
                             m_pProcCreateMonitor(NULL) , m_pProcTerminateMonitor(NULL) {}
...
class CProcCreateMonitor : public IWbemObjectSink
{
    public :
        CProcCreateMonitor( IWbemServices * aSvc , IUnsecuredApartment * aUnsecApp ) :
            m_lRef(0) , m_pSvc(aSvc) , m_pUnsecApp(aUnsecApp) , 
            m_pStub(NULL) , m_pSink(NULL) , m_pfnProc(NULL) {}
        ...
        virtual HRESULT STDMETHODCALLTYPE SetStatus( LONG aFlags , HRESULT aResult , BSTR aStrParam , IWbemClassObject __RPC_FAR * aObjParam );
