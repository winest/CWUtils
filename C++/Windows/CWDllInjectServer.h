...
#pragma pack( push, 8 )
    ...
    class InjectClientInfo
    {
        ...
        ~InjectClientInfo() { this->ClearNonReusablePart(); }
...
#pragma pack( pop )
    ...
    public:
    CDllInjectServer( HANDLE aActiveThread,
                      CONST WCHAR * aCfgPath,
                      size_t aRuleIndex,
                      CONST WCHAR * aRuleName,
                      UINT32 aLoadLibraryW32,
                      UINT32 aFreeLibrary32,
                      UINT64 aLoadLibraryW64,
                      UINT64 aFreeLibrary64 ) :
        m_hActiveThread( aActiveThread ),
        m_wstrCfgPath( aCfgPath ),
        m_uRuleIndex( aRuleIndex ),
        m_wstrRuleName( aRuleName ),
        m_uLoadLibraryW32( aLoadLibraryW32 ),
        m_uFreeLibrary32( aFreeLibrary32 ),
        m_uLoadLibraryW64( aLoadLibraryW64 ),
        m_uFreeLibrary64( aFreeLibrary64 ),
        m_bStarted( FALSE ),
        m_lJobThreadCnt( 0 ),
        m_hEvtNewJob( NULL ),
    ...
    std::map<DWORD, InjectClientInfo>
        m_mapClientStateTable;    //<key , value> = <PID , Interesting processes' information>

    HANDLE m_hPerServerEvt
        [PER_SERVER_EVT_COUNT];    //Per-server event. Used to communicate between remote client and local server
    HANDLE m_hPerServerMutex
        [PER_SERVER_MUTEX_COUNT];    //Per-server mutex. Used to ensure R2L(Remote to Local)'s share memory integrity
    HANDLE m_hPerServerSm
        [PER_SERVER_SM_COUNT];    //Per-server share memory handle. Used for data exchange (DLL_INJECT_SERVER_SM_DATA_HEADER_HEADER_HEADER). R2L is protected by mutex and L2R is protected by critical section
