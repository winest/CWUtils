...
typedef DWORD( CALLBACK * PFN_DLL_INJECT_CLIENT_INIT_CBK )( CONST WCHAR * aServerDirPath,
                                                            CONST WCHAR * aClientCfgPath );
typedef DWORD( CALLBACK * PFN_DLL_INJECT_CLIENT_DATA_RECV_CBK )( CHAR * aReqBuf,
                                                                 DWORD aReqBufSize,
                                                                 CHAR ** aRspBuf,
                                                                 DWORD * aRspBufSize );
...
class CDllInjectClient
{
    ...
    public:
    BOOL Connect( CONST WCHAR * aKey,
                  PFN_DLL_INJECT_CLIENT_INIT_CBK aInitCbk = NULL,
                  PFN_DLL_INJECT_CLIENT_DATA_RECV_CBK aDataCbk = NULL );
    BOOL Disconnect();
    BOOL IsConnected()
    {
        return InterlockedCompareExchange( reinterpret_cast<volatile LONG *>( &m_bConnected ), 0, 0 );
    }

    BOOL SendData( CHAR * aReqBuf, DWORD aReqBufSize, CHAR * aRspBuf, DWORD * aRspBufSize );

    HANDLE GetServerQuitEvt()
    {
        return ( this->IsConnected() ) ? (HANDLE)m_SmInit.InitReq.Remote.hPerServerEvt[PER_SERVER_EVT_INDEX_STOP]
                                       : NULL;
    }
    HANDLE GetClientQuitEvt()
    {
        return ( this->IsConnected() ) ? (HANDLE)m_SmInit.InitReq.Remote.hPerClientEvt[PER_CLIENT_EVT_INDEX_STOP]
                                       : NULL;
    }
    HANDLE GetServerAliveEvt()
    {
        return ( this->IsConnected() ) ? (HANDLE)m_SmInit.InitReq.Remote.hDllInjectMgrAliveThread : NULL;
    }

    LPTHREAD_START_ROUTINE GetFreeLibraryAddr()
    {
        return ( this->IsConnected() ) ? (LPTHREAD_START_ROUTINE)m_SmInit.InitReq.Local.pfnFreeLibrary : NULL;
    }
