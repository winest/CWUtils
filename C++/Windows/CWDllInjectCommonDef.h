...
#define GenerateShareMemoryName( aBuf, aPid )                                                    \
    do                                                                                           \
    {                                                                                            \
        WCHAR wzGuid[38] = {};                                                                   \
...
typedef struct _DLL_INJECT_SERVER_SM_INIT
{
    struct _InitReq
    {
        struct _Local
        {
            ...
            UINT64
                hRemoteProc;    //Remote process's handle. Job thread will use this to check whether the process exists instead of looking up ClientStateTable to speed up performance
            UINT64
                pfnFreeLibrary;    //Kernel32.dll's FreeLibrary address. This prevent FreeLibrary is hooked by others in the remote process
        } Local;

        struct _Remote
        {
            UINT64
                hEvtInitRsp;    //Per-client event. DuplicateHandle to DLL. Server wait this event and get status from InitRsp
            UINT64
                hDllInjectMgrAliveThread;    //Created in DllInjectMgr. DuplicateHandle to DLL. Used for monitoring DllInjectMgr's crash

            UINT64 hPerServerSm
                [PER_SERVER_SM_COUNT];    //Per-server share memory. DuplicateHandle to DLL. Used for data exchange (DLL_INJECT_SERVER_SM_DATA_HEADER)
            UINT64 hPerServerMutex
                [PER_SERVER_MUTEX_COUNT];    //Per-server mutex. DuplicateHandle to DLL. Used to ensure R2L's share memory integrity
...
{
    _DLL_INJECT_SERVER_SM_DATA_HEADER() :
        dwVersion( DLL_INJECT_SERVER_SM_DATA_HEADER_VER ),
        dwMaxSize( DLL_INJECT_SERVER_SM_DATA_HEADER_MAX_SIZE ),
        dwRemotePid( 0 ),
        hRemoteProc( NULL ),
        pLocalCtx( NULL )
    {
        ZeroMemory( &pReserved, sizeof( pReserved ) );
    }
    DWORD dwVersion;
    DWORD dwMaxSize;
