...
VOID TestChangeDaclPermission()
{
    ...
    if ( FALSE == CWUtils::ChangeDaclPermission( wstrPath.c_str(), SE_FILE_OBJECT, GENERIC_ALL, GRANT_ACCESS,
                                                 NO_INHERITANCE, L"winest", TRUSTEE_IS_NAME, TRUSTEE_IS_USER ) )
    {
        wstring wstrErr;
    ...
    else
    {
        ...
        {
            if ( FALSE == CWUtils::ChangeDaclPermission( wstrPath.c_str(), SE_FILE_OBJECT, GENERIC_ALL, GRANT_ACCESS,
                                                         NO_INHERITANCE, (CONST WCHAR *)pSid, TRUSTEE_IS_SID,
                                                         TRUSTEE_IS_GROUP ) )
            {
                wstring wstrErr;
                CWUtils::GetLastErrorStringW( wstrErr );
