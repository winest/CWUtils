...
LRESULT CALLBACK WndProc( HWND aHWnd, UINT aMsg, WPARAM aWParam, LPARAM aLParam )
{
    ...
    static WCHAR szBtnVolume[12][50] = { L"GetVolumes",
                                         L"GetVolumesPath",
                                         L"GetVolumesGuidPath",
                                         L"GetVolumePathFromGuidPath",
                                         L"GetGuidPathFromMountPath",
                                         L"GetMountPathFromFileOrDirPath",
                                         L"GetMountedFoldersFromGuidPath",
                                         L"GetVolumeType",
                                         L"GetVolumeInfo",
                                         L"GetVolumeInfo by handle",
                                         L"GetVolumeSpace",
                                         L"SetVolumeName" };
    ...
    switch ( aMsg )
    {
        case WM_CREATE:    //Receive when the window is created
        {
            ...
            hFont = CreateFont( -MulDiv( 12, GetDeviceCaps( hdc, LOGPIXELSY ), 72 ), 0, 0, 0, FW_BOLD, FALSE, FALSE,
                ...
                case BN_CLICKED:
                {
                    switch ( LOWORD( aWParam ) )
                    {
                        ...
                        case IDC_BTN_GET_MOUNTED_FOLDERS_FROM_GUID_PATH:
                        {
                            ...
                            {
                                int mountCount =
                                    CWUtils::GetMountedFoldersFromGuidPath( &wstrGuids[i * 50], (WCHAR **)paths );
                        ...
                        case IDC_BTN_GET_VOLUME_INFO:
                        {
                            ...
                            for ( i = 0; i < count; i++ )
                            {
                                BOOL ret = CWUtils::GetVolumeInfo( &wstrDrives[i * 4], wstrName, wstrFs, NULL, &serial,
                                                                   &maxFileLen );
                                if ( ret == TRUE )
                                {
                                    swprintf_s( buf, L"%lS - %12lS - %6lS - flag: %u - serial: %u - maxFileLen: %u\r\n",
                                                &wstrDrives[i * 4], wstrName.c_str(), wstrFs.c_str(), flag, serial,
                                                maxFileLen );
                                }
                                else
                                {
                        ...
                        case IDC_BTN_GET_VOLUME_SPACE:
                        {
                            ...
                            for ( i = 0; i < count; i++ )
                            {
                                ...
                                {
                                    swprintf_s(
                                        buf, L"%lS - %4I64uGb %4I64uMb - %4I64uGb %4I64uMb - %4I64uGb %4I64uMb\r\n",
                                        &wstrDrives[i * 4], free.QuadPart / dwGb, ( free.QuadPart % dwGb ) / dwMb,
        ...
        case WM_SIZE:    //Receive when windows size is changed
        {
            ...
            for ( i = 1; i < 12; i++ )
            {
                btnVolume[i].SetPos( btnVolume[i - 1].PosX(),
                                     btnVolume[i - 1].PosY() + btnVolume[i - 1].Height() + 10 );
            }
...
*/
    hWnd = CreateWindowExW( WS_EX_CLIENTEDGE, szClassName, L"Title of window", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
