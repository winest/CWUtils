...
CONST wstring g_TopWindowNames[] = { L"TApplication" , L"Internet Explorer_Server" , L"tSkMainForm" };
CONST wstring g_EdtWindowNames[] = { L"T*RichEdit*" };
CONST wstring g_BtnWindowNames[] = { L"TChatEntryControl" };
...
DWORD ScanUserData( CHAR * aUserData , UINT aUserDataSize );   //Return Win32 error code. Content will be blocked if return ERROR_CONTENT_BLOCKED
...
DWORD WINAPI RemoteHookHandler( VOID * pParam )
{
    ...
    if ( g_SmClient.Connect( wzSmName , FindWindowCallback ) )
    {
        ...
        while ( ! bStop )
        {
            ...
            switch ( dwWaitStop )
            {
                ...
                {
                    DbgOut( WARN , DBG_SKYPE_HANDLER , "PER_SERVER_EVT_INDEX_STOP event triggered" );
                    bStop = TRUE;
                    break;
                }
                case WAIT_OBJECT_0 + 1 :        //PER_CLIENT_EVT_INDEX_STOP
                {
                    DbgOut( WARN , DBG_SKYPE_HANDLER , "PER_CLIENT_EVT_INDEX_STOP event triggered" );
                    bStop = TRUE;
                    break;
                }
                case WAIT_OBJECT_0 + 2 :        //hDllInjectMgrAliveThread
                {
                    DbgOut( WARN , DBG_SKYPE_HANDLER , "CWDllInjectMgr leave event triggered" );
                    bStop = TRUE;
                    break;
                }
                default :
                {
                    DbgOut( ERRO , DBG_SKYPE_HANDLER , "WaitForMultipleObjects() return unexpected value 0x%08X. GetLastError()=%!WINERROR!" , dwWaitStop , GetLastError() );
                    bStop = TRUE;
                    break;
                }
            }
    ...
    {
        DbgOut( ERRO , DBG_SKYPE_HANDLER , "Connect() failed" );
    }
    ...
    {
        DbgOut( ERRO , DBG_SKYPE_HANDLER , "Cannot get FreeLibrary's address from share memory. Try to use 0x%p" , pfnFreeLibrary );
    ...
    if ( 0 < ATOMIC_READ(g_lDllRefCnt) && NULL != pfnFreeLibrary )
    {
        ...
        DbgOut( WARN , DBG_SKYPE_HANDLER , "Create thread to FreeLibrary. dwFreeTid=0x%04X, hFreeThread=0x%p" , dwFreeTid , hFreeThread );
        CloseHandle( hFreeThread );
    }

    DbgOut( INFO , DBG_SKYPE_HANDLER , "RemoteHookHandler() Leave" );
    return 0;
}
...
DWORD CALLBACK FindWindowCallback( CONST WCHAR * aServerDirPath , CONST WCHAR * aClientCfgPath )
{
    ...
    for ( UINT uRetryTimes = 0 ; uRetryTimes < FIND_TOP_WINDOW_RETRY_TIMES ; uRetryTimes++ ) 
    {
        ...
        {
            DbgOut( INFO , DBG_SKYPE_HANDLER , "Found Skype window. TID=0x%04X " , enumWndParam.dwMainTid );
            break;
        }
        else
        {
            DbgOut( ERRO , DBG_SKYPE_HANDLER , "Failed to find Skype window. dwMainTid=%lu" , enumWndParam.dwMainTid );
        }
    ...
    {
        DbgOut( WARN , DBG_SKYPE_HANDLER , "Stop finding Skype thread and leave" );
        dwRet = ERROR_RESOURCE_ENUM_USER_STOP;
        goto exit;
    }    

    //Hook on Skype's thread
    g_hHook = SetWindowsHookEx( WH_GETMESSAGE , HookCallback , NULL , enumWndParam.dwMainTid );
    if ( NULL == g_hHook )
    {
        DbgOut( ERRO , DBG_SKYPE_HANDLER , "SetWindowsHookEx() failed. GetLastError()=%!WINERROR!" , GetLastError() );
        dwRet = GetLastError();
        goto exit;
    }
    DbgOut( INFO , DBG_SKYPE_HANDLER , "SetWindowsHookEx() succeed. g_hHook=0x%p" , g_hHook );
...
BOOL CALLBACK EnumWindowCallback( HWND hWnd , LPARAM pEnumWndParam )
{
    ...
    if ( GetCurrentProcessId() == dwMainPid &&
         GetClassNameW( hWnd , wzClassName , _countof(wzClassName) ) )
    {
        DbgOut( INFO , DBG_SKYPE_HANDLER , "Checking window class name %ws" , wzClassName );
...
void UnInitialize()
{
    ...
    DbgOut( INFO , DBG_SKYPE_HANDLER , "Waiting for the hooking thread terminated" );
    if ( WAIT_TIMEOUT == WaitForSingleObject( g_hHookThread , 0 ) ) //Just log whether it's alive to avoid loader lock issue in DllMain()
    {
        DbgOut( ERRO , DBG_SKYPE_HANDLER , "Failed to wait until the hooking thread terminated. Unload directly" );
    }
    else
    {
        DbgOut( INFO , DBG_SKYPE_HANDLER , "Successfully wait until the hooking thread terminated" );
    }
...
DllMain( HMODULE aModule , DWORD aReason , LPVOID aReserved )
{
    ...
    switch ( aReason )
    {
        ...
        {
            WPP_INIT_TRACING( L"SkypeHandler" );
            DbgOut( VERB , DBG_SKYPE_HANDLER , "SkypeHandler.dll DLL_PROCESS_ATTACH" );
            ...
            {
                DbgOut( ERRO , DBG_SKYPE_HANDLER , "Failed to create remote hook thread. GetLastError()=%!WINERROR!" , GetLastError() );
                bRet = FALSE;
                break;
            }

            DbgOut( INFO , DBG_SKYPE_HANDLER , "Remote hook thread id 0x%04X created with handle 0x%p" , dwTid , g_hHookThread );
        ...
        {
            DbgOut( VERB , DBG_SKYPE_HANDLER , "SkypeHandler.dll DLL_PROCESS_DETACH" );
...
//     if ( WM_LBUTTONDOWN == msg->message || WM_LBUTTONDBLCLK == msg->message || WM_KEYDOWN == msg->message )
//     {
//         DbgOut( INFO , DBG_SKYPE_HANDLER , "message=0x%X, aCode=%d, aWParam=0x%X, aLParam=0x%X" , msg->message , aCode , aWParam , aLParam );
//     }

    //Monitor mouse left button up
    while ( ( WM_LBUTTONDOWN == msg->message || WM_LBUTTONDBLCLK == msg->message ) && PM_REMOVE == aWParam )
    {
        WCHAR wzWindowClassName[MAX_PATH] = { 0 };

        if ( 0 == GetClassNameW( msg->hwnd , wzWindowClassName , _countof( wzWindowClassName ) - 1 ) )
        {
            DbgOut( ERRO , DBG_SKYPE_HANDLER , "GetClassNameW() failed" );
            break;
        }
        else
        {
            DbgOut( INFO , DBG_SKYPE_HANDLER , "GetClassNameW() get %ws" , wzWindowClassName );
        }

        BOOL bBtnFound = FALSE;
        for ( size_t i = 0 ; i < _countof(g_BtnWindowNames) ; i++ )
        {
            if ( CWUtils::WildcardMatchW( wzWindowClassName , g_BtnWindowNames[i].c_str() ) )
            {
                DbgOut( INFO , DBG_SKYPE_HANDLER , "Class name %ws match %ws" , wzWindowClassName , g_BtnWindowNames[i].c_str() );
                bBtnFound = TRUE;
                break;
            }
        }
        if ( FALSE == bBtnFound )
        {
            DbgOut( VERB , DBG_SKYPE_HANDLER , "Class name not found. Skip it" );
            break;
        }

        HWND hWndChild = GetWindow( msg->hwnd , GW_CHILD );
        if ( NULL == hWndChild )
        {
            DbgOut( ERRO , DBG_SKYPE_HANDLER , "GetWindow() failed" );
            break;
        }

        //Check if the send button is pressed
        POINT ptMouse;
        ptMouse.x = LOWORD( msg->lParam );
        ptMouse.y = HIWORD( msg->lParam );
        ClientToScreen( msg->hwnd , &ptMouse );

        RECT rtChild;
        GetWindowRect( hWndChild , &rtChild );

        RECT rtCurrent;
        GetWindowRect( msg->hwnd , &rtCurrent );
        
        if ( FALSE == CheckSendButtonDown( ptMouse , rtChild , rtCurrent ) )
        {
            break;
        }
        DbgOut( INFO , DBG_SKYPE_HANDLER , "Get a mouse left button up of send button" );

        CHAR * pUserData = NULL;
        UINT uUserDataSize = 0;
        if ( GetUserData( hWndChild , &pUserData , &uUserDataSize ) )
        {
            dwScanResult = ScanUserData( pUserData , uUserDataSize );
            DbgOut( INFO , DBG_SKYPE_HANDLER , "Get scan result. dwScanResult=%lu" , dwScanResult );
            delete [] pUserData;
        }
        break;
    }

    if ( (WM_KEYDOWN == msg->message || WM_SYSKEYDOWN == msg->message) && VK_RETURN == msg->wParam && PM_REMOVE == aWParam )
    {
        //DbgOut( INFO , DBG_SKYPE_HANDLER , "Enter key message. aCode=%d, aWParam=%d, msg.wnd=%p, msg.aLParam=%d" , aCode , aWParam , msg->hwnd , msg->lParam );
        DbgOut( INFO , DBG_SKYPE_HANDLER , "Get a return key Message" );
    ...
    {
        DbgOut( WARN , DBG_SKYPE_HANDLER , "Block the message!" );
        msg->message = WM_USER;
        return 0;
    }
...
BOOL CheckSendButtonDown( CONST POINT & aMousePos , CONST RECT & aChildRect , CONST RECT & aCurrentRect )
{
    ...
    DbgOut( VERB , DBG_SKYPE_HANDLER , "Mouse point x:%d y:%d, Child left:%d top:%d right:%d bottom:%d, Current right:%d" ,
                                  aMousePos.x , aMousePos.y , aChildRect.left , aChildRect.top , aChildRect.right , aChildRect.bottom , aCurrentRect.right );
...
BOOL GetUserData( HWND aEditWnd , CHAR ** aUserData , UINT * aUserDataSize )
{
    ...
    DbgOut( INFO , DBG_SKYPE_HANDLER , "Try to find the message input window" );
    BOOL bFoundTarget = FALSE;
    while ( aEditWnd && FALSE == bFoundTarget )
    {
        if ( 0 == GetClassNameW( aEditWnd , wzWindowClassName , _countof( wzWindowClassName ) - 1 ) )
        {
            DbgOut( INFO , DBG_SKYPE_HANDLER , "GetClassNameW() failed!" );
            break;
        }
        else
        {
        ...
        for ( size_t i = 0 ; i < _countof(g_EdtWindowNames) ; i++ )
        {
            ...
            {
                DbgOut( INFO , DBG_SKYPE_HANDLER , "Class name %ws match %ws" , wzWindowClassName , g_EdtWindowNames[i].c_str() );
                bFoundTarget = TRUE;
                break;
            }
        }
    ...
    if ( bFoundTarget )
    {
        ...
        if ( nLen < 1 )
        {
            DbgOut( INFO , DBG_SKYPE_HANDLER , "Message length < 1, Skip it" );
            bFoundTarget = FALSE;
        }
        else
        {
            *aUserData = new (std::nothrow) CHAR[nLen + 1];
            if ( NULL != *aUserData )
            {
                ...
                *aUserDataSize = nLen + 1;
                DbgOut( INFO , DBG_SKYPE_HANDLER , "Get input message=%s" , *aUserData );
            }
        }
    }
    return bFoundTarget;
...
{
    DbgOut( VERB , DBG_SKYPE_HANDLER , "Enter" );
    ...
    if ( g_SmClient.SendData( aUserData , aUserDataSize , (CHAR *)&dwScanResult , &dwSize ) )
    {
        ...
        {
            DbgOut( ERROR , DBG_SKYPE_HANDLER , "Output size unexpected. dwSize=%lu" , dwSize );
            dwScanResult = ERROR_SUCCESS;
        }
    }
    else
    {
        DbgOut( ERROR , DBG_SKYPE_HANDLER , "SendData() failed. GetLastError()=%!WINERROR!" , GetLastError() );
        dwScanResult = ERROR_SUCCESS;
    }

    DbgOut( VERB , DBG_SKYPE_HANDLER , "Leave. dwScanResult=0x%08X" , dwScanResult );
    return dwScanResult;
}
