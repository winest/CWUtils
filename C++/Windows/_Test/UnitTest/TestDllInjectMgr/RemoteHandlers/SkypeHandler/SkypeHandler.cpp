...
DWORD WINAPI RemoteHookHandler( VOID * pParam )
{
    ...
    {
        DbgOut( ERRO, DBG_SKYPE_HANDLER, "Cannot get FreeLibrary's address from share memory. Try to use 0x%p",
                pfnFreeLibrary );
    ...
    if ( 0 < ATOMIC_READ( g_lDllRefCnt ) && NULL != pfnFreeLibrary )
    {
        ...
        DbgOut( WARN, DBG_SKYPE_HANDLER, "Create thread to FreeLibrary. dwFreeTid=0x%04X, hFreeThread=0x%p", dwFreeTid,
                hFreeThread );
...
BOOL APIENTRY DllMain( HMODULE aModule, DWORD aReason, LPVOID aReserved )
{
    ...
    switch ( aReason )
    {
        case DLL_PROCESS_ATTACH:
        {
            ...
            {
                DbgOut( ERRO, DBG_SKYPE_HANDLER, "Failed to create remote hook thread. GetLastError()=%!WINERROR!",
                        GetLastError() );
                bRet = FALSE;
                break;
            }

            DbgOut( INFO, DBG_SKYPE_HANDLER, "Remote hook thread id 0x%04X created with handle 0x%p", dwTid,
                    g_hHookThread );
...
LRESULT CALLBACK HookCallback( INT aCode, WPARAM aWParam, LPARAM aLParam )
{
    ...
    while ( ( WM_LBUTTONDOWN == msg->message || WM_LBUTTONDBLCLK == msg->message ) && PM_REMOVE == aWParam )
    {
        ...
        for ( size_t i = 0; i < _countof( g_BtnWindowNames ); i++ )
        {
            ...
            {
                DbgOut( INFO, DBG_SKYPE_HANDLER, "Class name %ws match %ws", wzWindowClassName,
                        g_BtnWindowNames[i].c_str() );
                bBtnFound = TRUE;
                break;
            }
        }
...
BOOL CheckSendButtonDown( CONST POINT & aMousePos, CONST RECT & aChildRect, CONST RECT & aCurrentRect )
{
    ...
    DbgOut( VERB, DBG_SKYPE_HANDLER, "Mouse point x:%d y:%d, Child left:%d top:%d right:%d bottom:%d, Current right:%d",
            aMousePos.x, aMousePos.y, aChildRect.left, aChildRect.top, aChildRect.right, aChildRect.bottom,
            aCurrentRect.right );
