...
DWORD WINAPI RemoteHookHandler( VOID * pParam )
{
    ...
    if ( g_SmClient.Connect( wzSmName , FindWindowCallback ) )
    {
        ...
        while ( ! bStop )
        {
            ...
            switch ( dwWaitStop )
            {
                ...
                default :
                {
                    DbgOut( ERRO , DBG_NOTEPAD_HANDLER , "WaitForMultipleObjects() return unexpected value 0x%08X. GetLastError()=%!WINERROR!" , dwWaitStop , GetLastError() );
                    bStop = TRUE;
                    break;
                }
            }
    ...
    {
        DbgOut( ERRO , DBG_NOTEPAD_HANDLER , "Cannot get FreeLibrary's address from share memory. Try to use 0x%p" , pfnFreeLibrary );
    ...
    if ( 0 < ATOMIC_READ(g_lDllRefCnt) && NULL != pfnFreeLibrary )
    {
        ...
        DbgOut( WARN , DBG_NOTEPAD_HANDLER , "Create thread to FreeLibrary. dwFreeTid=0x%04X, hFreeThread=0x%p" , dwFreeTid , hFreeThread );
...
DllMain( HMODULE aModule , DWORD aReason , LPVOID aReserved )
{
    ...
    switch ( aReason )
    {
        case DLL_PROCESS_ATTACH:
        {
            ...
            {
                DbgOut( ERRO , DBG_NOTEPAD_HANDLER , "Failed to create remote hook thread. GetLastError()=%!WINERROR!" , GetLastError() );
                bRet = FALSE;
                break;
            }

            DbgOut( INFO , DBG_NOTEPAD_HANDLER , "Remote hook thread id 0x%04X created with handle 0x%p" , dwTid , g_hHookThread );
...
BOOL CheckSendButtonDown( CONST POINT & aMousePos , CONST RECT & aChildRect , CONST RECT & aCurrentRect )
{
    ...
    DbgOut( VERB , DBG_NOTEPAD_HANDLER , "Mouse point x:%d y:%d, Child left:%d top:%d right:%d bottom:%d, Current right:%d" ,
...
BOOL GetUserData( HWND aEditWnd , CHAR ** aUserData , UINT * aUserDataSize )
{
    ...
    while ( aEditWnd && FALSE == bFoundTarget )
    {
        ...
        for ( size_t i = 0 ; i < _countof(g_EdtWindowNames) ; i++ )
        {
            ...
            {
                DbgOut( INFO , DBG_NOTEPAD_HANDLER , "Class name %ws match %ws" , wzWindowClassName , g_EdtWindowNames[i].c_str() );
                bFoundTarget = TRUE;
                break;
            }
        }
