...
VOID TestAutoBuffer()
{
    ...
    strncpy_s( autoBuf.GetBuf(), autoBuf.GetBufSize(), "123456789", _TRUNCATE );
    wprintf_s( L"%hs\n", autoBuf.GetBuf() );
    autoBuf[3] = 'X';
    wprintf_s( L"%hs\n", &autoBuf[0] );
...
VOID TestDynamicBuffer()
{
    ...
    buf.Append( (CONST UCHAR *)"123", 3 );
    wprintf_s( L"%hs\n", buf.GetBuf() );
    buf.Erase( 1, 5 );
    wprintf_s( L"%hs\n", buf.GetBuf() );
    buf.Insert( 0, (CONST UCHAR *)"456", 3 );
    wprintf_s( L"%hs\n", buf.GetBuf() );

    CWUtils::CDynamicBuffer buf2( buf );
    wprintf_s( L"%hs\n", buf2.GetBuf() );
    buf2 += buf;
    wprintf_s( L"%hs\n", &buf2[0] );
...
VOID TestMemFileBuffer()
{
    ...
    memFileBuf << "123";
    wprintf_s( L"%c\n", memFileBuf[1] );
    memFileBuf.Read( 0, (UCHAR *)szBuf, sizeof( szBuf ), bEnd );
    wprintf_s( L"%hs. bEnd=%d\n", szBuf, bEnd );
    memFileBuf << strBuf;
    wprintf_s( L"%hs\n", strBuf.c_str() );
    memFileBuf >> strBuf;
    wprintf_s( L"%hs\n", strBuf.c_str() );
