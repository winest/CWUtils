    ...
    switch( aMsg )
    {
        case WM_CREATE :    //Receive when the window is created
        {
            ...
            for ( i = 0 ; i < EDT_MAIN_COUNT ; i++ )
            {
                ...
                {
                    ((CEdit *)g_ctrlMain[EDT_INPUT + i])->Init( IDC_EDT_INPUT + i , aHWnd , hInstance , szEdt[i] ,
                                                        0 , 0 , 0 , 0 , WS_CHILD | WS_VISIBLE | WS_BORDER | ES_LEFT | 
        ...
        case WM_SIZE :    //Receive when windows size is changed
        {
            ...
            g_ctrlMain[LAB_INPUT]->SetPos( nWidth / 2 - g_ctrlMain[EDT_INPUT]->Width() - 10 - g_ctrlMain[LAB_INPUT]->Width() , 10 );
            g_ctrlMain[EDT_INPUT]->SetPos( g_ctrlMain[LAB_INPUT]->PosX() + g_ctrlMain[LAB_INPUT]->Width() + 10 , g_ctrlMain[LAB_INPUT]->PosY() - 2 );
            g_ctrlMain[BTN_START]->SetPos( g_ctrlMain[EDT_INPUT]->PosX() + g_ctrlMain[EDT_INPUT]->Width() + 10 , g_ctrlMain[LAB_INPUT]->PosY() - 2 );

            g_ctrlMain[BTN_CLEAN]->SetPos( nWidth - 10 - g_ctrlMain[BTN_CLEAN]->Width() , g_ctrlMain[LAB_INPUT]->PosY() + g_ctrlMain[LAB_INPUT]->Height() + 10 );
...
INT WINAPI WinMain( HINSTANCE aHInstance , HINSTANCE aHPrevInstance , LPSTR aCmdLine , INT aCmdShow )
{
    ...
    hWnd = CreateWindowExW( WS_EX_CLIENTEDGE , szClassName , L"Main Window" , WS_OVERLAPPEDWINDOW ,
