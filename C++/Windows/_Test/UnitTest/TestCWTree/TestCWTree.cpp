...
typedef struct _NODE
{
    ...
    _NODE operator+( _NODE & aRight )
    {        
        return _NODE( this->value + aRight.value , this->weight + aRight.weight );
    }
...
void PrintBinaryTree( CWUtils::CBinaryTree<NODE> * aTree )
{
    ...
    while ( true )
    {
        do
        {
            ...
            double width = ( pow( (double)2 , aTree->m_nHeight-1 ) +1) / ( pow( (double)2 , aTree->m_nHeight - printTree->m_nHeight )+1 );
...
VOID TestBinaryTree()
{
    ...
    node.m_value.value = 1;    node.m_value.weight = 2;
    tree.SetLeft( tree , node );
    node.m_value.value = 3;    node.m_value.weight = 4;
    tree.SetRight( tree , node );

    node.m_value.value = 5;    node.m_value.weight = 6;
    tree.SetLeft( *tree.m_left , node );
    node.m_value.value = 7;    node.m_value.weight = 8;
    tree.SetRight( *tree.m_left , node );
    
    node.m_value.value = 9;    node.m_value.weight = 10;
    tree.SetLeft( *tree.m_right , node );
    node.m_value.value = 11;    node.m_value.weight = 12;
