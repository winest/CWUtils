...
#    define TRACE_LEVEL_MUST 1           // Message that must be shown
#    define TRACE_LEVEL_ERROR 2          // Severe errors that need logging
#    define TRACE_LEVEL_WARNING 3        // Warnings such as allocation failure
#    define TRACE_LEVEL_INFORMATION 4    // Includes non-error cases(e.g.,Entry-Exit)
#    define TRACE_LEVEL_VERBOSE 5        // Detailed traces from intermediate steps
#    define TRACE_LEVEL_FUNCTION 6       // Function enter and leave
#    define TRACE_LEVEL_NOSY 7           // All detailed such as packet content
#    define TRACE_LEVEL_RESERVED8 8
#    define TRACE_LEVEL_RESERVED9 9
#endif

//Just a shorter version
#define MUST TRACE_LEVEL_MUST
...
#define FUNC TRACE_LEVEL_FUNCTION
#define NOSY TRACE_LEVEL_NOSY
...
#define WPP_LEVEL_FLAGS_ENABLED( level, flags ) \
    ( WPP_FLAG_ENABLED( flags ) && WPP_CONTROL( WPP_BIT_##flags ).Level >= level )
#define WPP_LEVEL_FLAGS_LOGGER( level, flags ) WPP_FLAG_LOGGER( flags )



//Below comments will be parsed by WPP, don't remove comments
// begin_wpp config
// FUNC FuncEnter{LEVEL=FUNC}(FUNC_FLAGS);
// FUNC FuncLeave{LEVEL=FUNC}(FUNC_FLAGS);
// USESUFFIX(FuncEnter, "%!FUNC!() Enter");
// USESUFFIX(FuncLeave, "%!FUNC!() Leave");
// end_wpp

#define WPP_LEVEL_FUNC_FLAGS_ENABLED( level, flags ) \
    ( WPP_FLAG_ENABLED( flags ) && WPP_CONTROL( WPP_BIT_##flags ).Level >= FUNC )
#define WPP_LEVEL_FUNC_FLAGS_LOGGER( level, flags ) WPP_FLAG_LOGGER( flags )
...
#define WPP_CONTROL_GUIDS                                                                                          \
    WPP_DEFINE_CONTROL_GUID( WppLogGuid, ( 84B0E5DA, 6A9F, 4124, B960, EBC6507AFC1B ),                             \
                             WPP_DEFINE_BIT( DBG_TEST ) WPP_DEFINE_BIT( DBG_DLL_INJECT_MGR )                       \
                                 WPP_DEFINE_BIT( DBG_DLL_INJECT_MGR_HELPER ) WPP_DEFINE_BIT( DBG_NOTEPAD_HANDLER ) \
                                     WPP_DEFINE_BIT( DBG_SKYPE_HANDLER ) WPP_DEFINE_BIT( DBG_UTILS ) )
