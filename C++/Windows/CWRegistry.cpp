                   ...
                   DWORD dwOption )
{
    ...
    DWORD dw;
    if ( ERROR_SUCCESS ==
         RegCreateKeyExW( aRegKey, aSection, 0, REG_NONE, dwOption, KEY_WRITE | KEY_READ, NULL, &hKey, &dw ) )
    {
        LONG lResult = RegSetValueExW( hKey, aValueName, NULL, REG_SZ, (LPBYTE)aValue,
                                       static_cast<DWORD>( wcslen( aValue ) * sizeof( WCHAR ) + sizeof( WCHAR ) ) );
        RegCloseKey( hKey );
        return ( ERROR_SUCCESS == lResult );
    }
    return FALSE;
                        ...
                        DWORD dwOption )
{
         ...
         RegCreateKeyExW( aRegKey, aSection, 0, REG_NONE, dwOption, KEY_WRITE | KEY_READ, NULL, &hKey, &dw ) )
    {
        ...
        *( string_param + pos_current ) = 0;
        LONG result = RegSetValueExW( hKey, aValueName, NULL, REG_MULTI_SZ, (LPBYTE)string_param,
                                      static_cast<DWORD>( total_length * sizeof( WCHAR ) ) );
        delete[] string_param;
        RegCloseKey( hKey );
...
BOOL GetRegMultiString( HKEY aRegKey, CONST WCHAR * aSection, CONST WCHAR * aValueName, vector<wstring> & aValues )
{
    ...
    if ( ERROR_SUCCESS == RegOpenKeyExW( aRegKey, aSection, 0, KEY_READ, &hKey ) )
    {
        ...
        if ( ERROR_SUCCESS == result )
        {
            ...
            while ( ( pos_current = ( size_t )( wcschr( string_param + pos_previous, '\0' ) - string_param ) ) <
                    total_string_length )
            {
...
BOOL RemoveRegPendingFileRename( CONST vector<wstring> & aFilenames )
{
    ...
    if ( TRUE == IsRegKeyExists( HKEY_LOCAL_MACHINE, REG_KEY_SESSION_MANAGER ) )
    {
        ...
        {
            if ( TRUE == GetRegMultiString( HKEY_LOCAL_MACHINE, REG_KEY_SESSION_MANAGER, REG_VAL_PENDING_FILE_RENAME_OP,
                                            multi_string ) )
            {
                ...
                {
                    std::transform( aFilenames[i].begin(), aFilenames[i].end(), vecLowerFilenames[i].begin(),
                                    ::towlower );
                }
                ...
                if ( bModified )
                {
                    if ( SetRegMultiString( HKEY_LOCAL_MACHINE, REG_KEY_SESSION_MANAGER, REG_VAL_PENDING_FILE_RENAME_OP,
                                            multi_string ) )
                    {
