...
NTSTATUS _GetRegRaw( IN CONST WCHAR * aKeyPath,
                     IN CONST WCHAR * aValueName,
                     IN ULONG aType,
                     OUT VOID * aBuf,
                     IN OUT ULONG * aBufSize )
{
    NT_ASSERT( aBufSize );
    DbgOut( VERB, DBG_KM_UTILS, "Enter. aKeyPath=%ws, aValueName=%ws, aType=%lu, aBuf=0x%p, aBufSize=%lu", aKeyPath,
            aValueName, aType, aBuf, *aBufSize );
    ...
    if ( NT_SUCCESS( status ) )
    {
        ...
        status =
            ZwQueryValueKey( hKey, &usValueName, KeyValuePartialInformation, pResult, sizeof( szTmpBuf ), &ulRetSize );
        if ( NT_SUCCESS( status ) )
        {
            DbgOut( INFO, DBG_KM_UTILS,
                    "ZwQueryValueKey() succeed. TitleIndex=%lu, Type=%lu, Data=0x%p, DataLength=%lu",
        ...
        {
            DbgOut( ERRO, DBG_KM_UTILS, "ZwQueryValueKey() failed. usValueName=%wZ, status=%!STATUS!", &usValueName,
                    status );
        }
        ZwClose( hKey );
    }
...
NTSTATUS GetRegFlag( IN CONST VOID * aRoot,
                     IN CONST WCHAR * aKeyPath,
                     IN CONST WCHAR * aFlagName,
                     OUT ULONG32 * aFlagVal )
{
    ...
    else
    {
        ...
        } rootMap[] = { { HKEY_LOCAL_MACHINE, L"\\Registry\\Machine\\" }, { HKEY_USERS, L"\\Registry\\User\\" } };
        ...
        for ( size_t i = 0; i < _countof( rootMap ); i++ )
        {
            if ( rootMap[i].hRoot == aRoot )
            {
                ...
                usFullKeyPath.MaximumLength =
                    ( USHORT )( ( wcslen( rootMap[i].wzRootReg ) + wcslen( aKeyPath ) + 1 ) * sizeof( WCHAR ) );
                usFullKeyPath.Buffer =
                    (WCHAR *)ExAllocatePoolWithTag( NonPagedPool, usFullKeyPath.MaximumLength, CW_MEM_TAG_UTILS );
                if ( NULL == usFullKeyPath.Buffer )
                {
                    DbgOut( ERRO, DBG_KM_UTILS, "ExAllocatePoolWithTag() failed. usFullKeyPath.MaximumLength=%hu",
                            usFullKeyPath.MaximumLength );
                    break;
                }
