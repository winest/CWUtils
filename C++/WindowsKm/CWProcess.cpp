...
typedef NTSTATUS (__stdcall * PFN_ZW_QUERY_SYSTEM_INFORMATION)( IN ULONG aSystemInformationClass , IN OUT PVOID aSystemInformation , 
                                                                IN ULONG aSystemInformationLength , OPTIONAL OUT PULONG aReturnLength );

typedef NTSTATUS (__stdcall * PFN_ZW_QUERY_INFORMATION_PROCESS)( IN HANDLE aProcessHandle , IN PROCESSINFOCLASS aProcessInformationClass ,
                                                                 OUT PVOID aProcessInformation , IN ULONG aProcessInformationLength , OPTIONAL OUT PULONG aReturnLength );
...
NTSTATUS GetProcessPathByHandle( HANDLE aProcessHandle , PUNICODE_STRING aProcessPath )
{
    ...
    PFN_ZW_QUERY_INFORMATION_PROCESS ZwQueryInformationProcess = (PFN_ZW_QUERY_INFORMATION_PROCESS) MmGetSystemRoutineAddress( &funcName );
...
NTSTATUS GetPidByProcessName( IN CONST PUNICODE_STRING aImagePath , IN OUT PHANDLE aPid )
{
    ...
    PFN_ZW_QUERY_SYSTEM_INFORMATION ZwQuerySystemInformation = (PFN_ZW_QUERY_SYSTEM_INFORMATION) MmGetSystemRoutineAddress( &funcName );
    ...
    if ( STATUS_INFO_LENGTH_MISMATCH != status )
    {
        DbgOut( ERRO , DBG_KM_UTILS , "ZwQuerySystemInformation() failed. status=%!STATUS!, ulByteNeed=%lu" , status , ulByteNeed );
        goto exit;
    }
    ...
    if ( ! NT_SUCCESS(status) )
    {
        DbgOut( ERRO , DBG_KM_UTILS , "Failed to allocate buffer for ZwQuerySystemInformation(). status=%!STATUS!, ulByteNeed=%lu" , status , ulByteNeed );
        goto exit;
    }    

    status = STATUS_NOT_FOUND;
    for ( PSYSTEM_PROCESS_INFORMATION pProcInfo = pInfo ; 0 < pProcInfo->NextEntryOffset ; pProcInfo = (PSYSTEM_PROCESS_INFORMATION)(((PUCHAR)pProcInfo) + pProcInfo->NextEntryOffset) )
    {
