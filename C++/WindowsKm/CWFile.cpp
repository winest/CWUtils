...
NTSTATUS DevicePathToFilePath( IN CONST PUNICODE_STRING aDevicePath, OUT PUNICODE_STRING aFilePath )
{
    ...
    for ( WCHAR wLetter = L'A'; wLetter <= L'Z'; wLetter++ )
    {
        ...
        ULONG ulRetSize;
        InitializeObjectAttributes( &objDevicePath, &usSymbolicName, OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE, NULL,
                                    NULL );
        ...
        DbgOut( INFO, DBG_KM_UTILS, "usDeviceName: ulRetSize=%lu, Length=%hu, Buffer=%wZ", ulRetSize,
                usDeviceName.Length, &usDeviceName );
        ...
        if ( STATUS_BUFFER_TOO_SMALL == status )
        {
            ...
            usDeviceName.Buffer =
                (WCHAR *)ExAllocatePoolWithTag( NonPagedPool, usDeviceName.MaximumLength, CW_MEM_TAG_UTILS );
            status = ZwQuerySymbolicLinkObject( hSymbolic, &usDeviceName, &ulRetSize );
        ...
        else if ( 0 != _wcsnicmp( usDeviceName.Buffer, aDevicePath->Buffer,
                                  min( usDeviceName.Length, aDevicePath->Length ) / sizeof( WCHAR ) ) )
        {
            ZwClose( hSymbolic );
            continue;
        }
        else
        {
            ...
            {
                DbgOut( WARN, DBG_KM_UTILS, "Buffer too small. aFilePath->MaximumLength=%hu, uFilePathSize=%hu",
            ...
            if ( NT_SUCCESS( status ) )
            {
                ...
                RtlUnicodeStringCbCatStringN( aFilePath, &aDevicePath->Buffer[usDeviceName.Length / sizeof( WCHAR )],
                                              aDevicePath->Length - usDeviceName.Length );
                DbgOut( INFO, DBG_KM_UTILS, "aFilePath: Length=%hu, MaximumLength=%hu, Buffer=%wZ", aFilePath->Length,
                        aFilePath->MaximumLength, aFilePath );
            }
...
NTSTATUS FilePathToDevicePath( IN CONST PUNICODE_STRING aFilePath, OUT PUNICODE_STRING aDevicePath )
{
    ...
    usSymbolicName.Buffer =
        (WCHAR *)ExAllocatePoolWithTag( NonPagedPool, usSymbolicName.MaximumLength, CW_MEM_TAG_UTILS );
    ...
    ULONG ulRetSize;
    status = ZwQuerySymbolicLinkObject( hSymbolic, &usDeviceName, &ulRetSize );
    DbgOut( INFO, DBG_KM_UTILS, "usDeviceName: ulRetSize=%lu, Length=%hu, Buffer=%wZ", ulRetSize, usDeviceName.Length,
            &usDeviceName );
    ...
    {
        DbgOut( WARN, DBG_KM_UTILS, "Buffer too small. aDevicePath->MaximumLength=%hu, uDevicePathSize=%hu",
    ...
    if ( NT_SUCCESS( status ) )
    {
        ...
        RtlUnicodeStringCbCatStringN( aDevicePath, &aFilePath->Buffer[uSymbolicLen],
                                      aFilePath->Length - ( uSymbolicLen * sizeof( WCHAR ) ) );
