...
typedef enum _FtdTagType : UINT8
{
    FTD_TAG_TYPE_NONE = 0 ,
...
{
    VOID Reset() { uFtdTagType = FTD_TAG_TYPE_NONE; uFtdTagSize = 0; strFtdTagContent.clear(); }
...
{
    _FtdPartIdInfo() {}
    _FtdPartIdInfo( FtdPartId aPartId , std::string aPartIdDesc ,
                    FtdPartDataType aPartDataType , UINT aPartSize , FtdPartId aNextPartId ) :
                    uPartId(aPartId) , strPartIdDesc(aPartIdDesc) ,
                    uPartDataType(aPartDataType) , uPartSize(aPartSize) , uNextPartId(aNextPartId) {}
    FtdPartId   uPartId;
...
class CFtdParser;
typedef BOOL (CALLBACK *FtdRecordParsedCbk)( IN VOID * aContext , IN BOOL aConnOut , IN CONST CWUtils::CFtdParser * aFtdParser );
...
class CFtdParser
{
    ...
    private :
        ...
        typedef struct _MainPartParsedInfo
        {
            ...
            VOID Reset() { uMainPartParsedSize = 0; uCompressionParsedSize = 0; vecDecompressedBuf.clear(); bIsEscaping = FALSE; }
        ...
        {
            _ParseParam() : bConnOut(FALSE) , pBuf(NULL) , uBufSize(0) , pMainPartParsedInfo(NULL) , pSubPartParsedInfo(NULL) , pFieldMgr(NULL) {}
            BOOL bConnOut;
    ...
    protected :
        ...
        FtdParserErr DoParse( ParseParam * aParam , UINT aMainPartSize , FtdPartId aBeginState , FtdPartId aEndState , BOOL aUseDecompessedBuf );
