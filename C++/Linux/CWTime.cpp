...
void StopWatch::Pause() {
  if (m_nState == StopWatchState::STOP_WATCH_STATE_START ||
...
void StopWatch::Stop() {
  if (m_nState != StopWatchState::STOP_WATCH_STATE_STOP) {
    ...
    m_vecAllInt.push_back(m_timeLastInt.tv_sec * NS_PER_SEC +
                          m_timeLastInt.tv_nsec);
...
double StopWatch::GetAvgIntervalInNano() const {
  return (double)(m_timeTotalInt.tv_sec * NS_PER_SEC + m_timeTotalInt.tv_nsec) /
         m_vecAllInt.size();
}
...
uint64_t StopWatch::Get1QIntervalInNano() {
  ...
  std::nth_element(m_vecAllInt.begin(), m_vecAllInt.begin() + nFirstQuartilePos,
                   m_vecAllInt.end());
...
double StopWatch::GetMedianIntervalInNano() {
  ...
  if (m_vecAllInt.size() % 2 == 0) {
    return static_cast<double>(m_vecAllInt[m_vecAllInt.size() / 2 - 1] +
                               m_vecAllInt[m_vecAllInt.size() / 2]) /
           2;
  } else {
...
uint64_t StopWatch::Get3QIntervalInNano() {
  ...
  std::nth_element(m_vecAllInt.begin(), m_vecAllInt.begin() + nThirdQuartilePos,
                   m_vecAllInt.end());
