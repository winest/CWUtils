...
class CCriticalSection : public CLock
{
    ...
    inline virtual ~CCriticalSection() { pthread_mutex_destroy( &m_mutex ); }

    inline BOOL Lock() { return ( 0 == pthread_mutex_lock( &m_mutex ) ) ? TRUE : FALSE; }
    inline BOOL Unlock() { return ( 0 == pthread_mutex_unlock( &m_mutex ) ) ? TRUE : FALSE; }
...
{
    public:
    CRWLockPthread() { pthread_rwlock_init( &m_rwLock, NULL ); }
    virtual ~CRWLockPthread() { pthread_rwlock_destroy( &m_rwLock ); }
