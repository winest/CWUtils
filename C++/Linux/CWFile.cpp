...
BOOL _WStringToString(IN CONST std::wstring &aWString, OUT std::string &aString,
                      DWORD aCodePage) {
  UNREFERENCED_PARAMETER(
      aCodePage); // Linux will use the locale in setlocale( LC_CTYPE , NULL )
...
BOOL CFile::WriteLine(CONST UCHAR *aData, SIZE_T aDataSize) {
  ...
  if (FALSE != bRet) {
    bRet =
        this->Write((CONST UCHAR *)m_strLineSep.c_str(), m_strLineSep.size());
  }
  return bRet;
}
